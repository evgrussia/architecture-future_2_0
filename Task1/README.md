...
```mermaid
flowchart TB
    OP["Клиентский интерфейс оператора<br/>(PowerBuilder)"]
    DWH[("DWH<br/>SQL Server 2008<br/>данные + бизнес-логика")]
    BI["BI-система<br/>(Power BI + кастомизации)"]
    ESB{{"ESB<br/>Apache Camel"}}
    AI["ИИ-сервисы<br/>(Python)"]
    FIN["Финтех-сервисы<br/>(Golang / Java)"]
    INT["Внутренние сервисы"]

    OP -- "медданные, снимки" --> DWH
    DWH -- "данные" --> BI
    DWH <--> ESB
    ESB <-- "медданные" --> AI
    ESB <-- "финансовые данные" --> FIN
    ESB <-- "внутр. финансы, управление клиниками" --> INT
```
...
flowchart TB
    %% ===== Акторы =====
    USER(["👤 Бизнес-пользователь домена<br/>строит и конструирует отчёты"])
    OPER(["👤 Оператор клиники"])
    OWNER(["👤 Владелец домена / Data-инженер"])
    PARTNER(["👤 Внешний партнёр<br/>фарма / электроника (план)"])

    subgraph CLOUD["☁️ Облачная платформа Будущее 2.0"]
        direction TB

        %% ===== Контур самообслуживания =====
        subgraph ACCESS["Контур самообслуживания и доступа"]
            direction TB
            GW["API Gateway<br/>[маршрутизация, троттлинг]"]
            PORTAL["Портал самообслуживания<br/>(SPA, веб)<br/>каталог отчётов, конструктор"]
            BIENG["BI / Query-сервис<br/>(cloud BI)<br/>интерактивные отчёты по gold-слою"]
        end

        %% ===== Платформа данных (Lakehouse) =====
        subgraph LAKE["Платформа данных — Data Lakehouse"]
            direction TB
            STORE[("Объектное хранилище<br/>Data Lake: bronze/silver/gold")]
            COMPUTE["Compute / Query Engine<br/>(Spark + Trino/ClickHouse)"]
            ORCH["Оркестратор пайплайнов<br/>(Airflow)"]
            CATALOG["Data Catalog + Governance<br/>метаданные, lineage, контракты данных"]
            POLICY["Сервис политик доступа к данным<br/>row/column-level security, маскирование"]
        end

        %% ===== Доменные data products (Data Mesh) =====
        subgraph DOMAINS["Доменные data products (Data Mesh)"]
            direction LR
            DCLINIC["Домен «Клиника»<br/>операционные данные<br/>(без медкарт для аналитики)"]
            DFIN["Домен «Финтех»<br/>сервисы Go/Java + data product"]
            DAI["Домен «ИИ-сервисы»<br/>Python, изолированный мед-контур"]
            DHQ["Домен «Головной офис»<br/>сводные KPI, фин. отчётность"]
            DNEW["Будущие домены<br/>«Фарма», «Электроника»"]
        end

        %% ===== Интеграция =====
        subgraph INTEG["Интеграционный слой"]
            direction TB
            KAFKA{{"Событийная шина<br/>Apache Kafka<br/>(event backbone)"}}
            CDC["CDC-коннекторы<br/>(Debezium) из источников"]
        end

        %% ===== Сквозные сервисы =====
        subgraph CROSS["Сквозные сервисы"]
            direction TB
            IAM["IAM / SSO<br/>(Keycloak)<br/>RBAC / ABAC"]
            SEC["Безопасность и комплаенс<br/>шифрование, аудит, DLP"]
            BACKUP["Резервное копирование<br/>гео-репликация"]
        end
    end

    %% ===== Легаси-контур (выводится постепенно) =====
    subgraph LEGACY["Легаси-контур (временно, ≤1 года)"]
        direction TB
        DWH[("DWH SQL Server 2008<br/>(strangler: вывод логики)")]
        OPUI["Интерфейс оператора<br/>(PowerBuilder)"]
        ESB{{"ESB Apache Camel<br/>(остаточная интеграция)"}}
    end

    %% ===== Защищённое мед-хранилище =====
    MEDVAULT[("🔒 Защищённое мед-хранилище<br/>медкарты, истории, снимки<br/>НЕ попадает в витрину аналитики")]

    %% ===== Связи: пользователи =====
    USER --> PORTAL
    OPER --> OPUI
    OWNER --> CATALOG
    PARTNER -. "через API Gateway (план)" .-> GW
    PORTAL --> GW
    GW --> BIENG

    %% ===== Связи: доступ к данным =====
    BIENG -- "запросы к gold" --> COMPUTE
    BIENG -- "проверка прав" --> POLICY
    POLICY -- "решения по доступу" --> IAM
    PORTAL -- "поиск данных" --> CATALOG
    ...
    DCLINIC -- "data product → silver/gold" --> STORE
    DFIN -- "data product → silver/gold" --> STORE
    DAI -- "data product → silver/gold" --> STORE
    DHQ -- "сводные витрины" --> STORE
    DNEW -. "подключение по модели Data Mesh" .-> STORE

    %% ===== Связи: интеграция / поступление данных =====
    CDC -- "сырые данные → bronze" --> STORE
    KAFKA --> CDC
    DFIN <--> KAFKA
    DAI <--> KAFKA
    DCLINIC <--> KAFKA

    %% ===== Связи: легаси и strangler =====
    DWH -- "CDC миграция данных" --> CDC
    ESB <--> KAFKA
    OPUI --> DWH
    DWH <--> ESB

    %% ===== Связи: мед-данные изолированы =====
    OPER -. "медданные, снимки" .-> MEDVAULT
    DAI -- "доступ к медданным<br/>(изолированно, под аудитом)" --> MEDVAULT
    MEDVAULT --- SEC
    ...
    classDef legacy fill:#ffe0e0,stroke:#c00,stroke-width:1px;
    classDef vault fill:#fff3cd,stroke:#d39e00,stroke-width:2px;
    class DWH,OPUI,ESB legacy;
    class MEDVAULT vault;
```
